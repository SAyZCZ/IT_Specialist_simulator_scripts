# Read opening_stock, delivered_items, issued_items and damaged_items.
# Calculate available_items and usable_items.
# Print both results.


#1. Read four values separately with input() and convert them to numbers:
   opening_stock  = int(input())    #stock before today's operations,
   delivered_items = int(input()) #number of received products,
   issued_items  = int(input())    #number of issued products,
   damaged_items  = int(input())   #damaged products remaining in the current stock.
...
print(available_items,usable_items)
